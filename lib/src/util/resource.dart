...
/// ResourceFailure is a wrapper over the exception thrown by the mobile BE.
abstract class Resource<T1> {
  ...
  Resource<OtherType> castFailure<OtherType>() {
    ...
    throw ArgumentError('Cannot cast ResourceSuccess to another type');
  }
}

/// A wrapper over the data returned by the mobile backend.
/// Signifies a successful scenario.
...
/// A wrapper over an error thrown by the mobile backend.
/// Signifies that something went wrong when executing the operation.
...
extension ResourceDataExtensions<T1> on T1 {
  ...
  Resource<T1> toResourceSuccess() {
    if (this is Error) {
      throw ArgumentError('Cannot convert Error to ResourceSuccess; '
          'convert to ResourceFailure instead');
    }

    if (this is Exception) {
      throw ArgumentError('Cannot convert Exception to ResourceSuccess; '
          'convert to ResourceFailure instead');
    }
